...
1. 使用**db_ets:reg/4**注册要管理的ETS表，或通过**db_ets:reg_select/5**注册要管理的ETS表，并查找数据并自动插入ETS表

2. 使用**db_ets:init_insert/2**插入数据库中已存在的数据

3. 使用**db_ets:flush/1**更新同步数据到数据库表中

4. 使用**db_ets:pull/1**获取脏数据列表

5. 实例1，通过**db_ets:reg/4**管理ETS表
  ...
  ets:update_element(ets_ets_table, 1, {#table_1.field_2, 200}),
  ok.
  ```
